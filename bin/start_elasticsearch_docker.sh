...
if [ ${containerRuns} -gt 0 ]; then
  ...
  docker run -d --restart=unless-stopped --name yacy-grid-elasticsearch -p 127.0.0.1:9200:9200 -p 127.0.0.1:9300:9300 -v `pwd`/../conf/elasticsearch.yml:/usr/share/elasticsearch/config/elasticsearch.yml -v yacy-grid-elasticsearch:/usr/share/elasticsearch/data elasticsearch:6.8.20
fi
