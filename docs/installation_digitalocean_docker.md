...
4. Docker is already installed, you can check it using ```docker version``` command. You should see something like this:
   ...
   Server:
    ...
    OS/Arch:      linux/amd64
   ```
5. Pull Docker image from [Yacy repository](https://hub.docker.com/r/nikhilrayaprolu/yacygridmcp/) in Docker Hub (it should take about a minute):
   ```bash
...
6. OK, you're ready to run yacy:
   ```bash
   docker run -d -p 8100:8100 -p 443:443 nikhilrayaprolu/yacygridmcp:latest
   ```
